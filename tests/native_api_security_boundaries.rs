...
fn write_zip_file_entry(
    ...
    writer
        .write_all(contents)
        .expect("failed to write zip file entry contents");
}
...
fn mark_first_zip_entry_as_symlink(zip_path: &Path) {
    ...
    let Some(header_start) = archive_bytes
        .windows(4)
...
fn unzip_rejects_parent_traversal_entries() {
    let (project_root, _, output) = run_unzip_script_with_archive(
        "native_api_unzip_parent_traversal",
        |zip_path| {
...
fn unzip_rejects_absolute_entries() {
    let (_, _, output) = run_unzip_script_with_archive("native_api_unzip_absolute_path", |zip_path| {
        create_zip_archive(zip_path, |writer| {
...
fn unzip_rejects_archives_exceeding_single_entry_limit() {
    let (_, _, output) = run_unzip_script_with_archive("native_api_unzip_single_limit", |zip_path| {
        create_zip_archive(zip_path, |writer| {
...
fn unzip_rejects_archives_exceeding_total_size_limit() {
    let (_, _, output) = run_unzip_script_with_archive("native_api_unzip_total_limit", |zip_path| {
        create_zip_archive(zip_path, |writer| {
...
fn unzip_rejects_archives_exceeding_entry_count_limit() {
    let (_, _, output) = run_unzip_script_with_archive("native_api_unzip_entry_count_limit", |zip_path| {
        create_zip_archive(zip_path, |writer| {
            for index in 0..1025 {
...
fn unzip_extracts_safe_nested_entries() {
    let (project_root, output_dir, output) = run_unzip_script_with_archive(
        "native_api_unzip_safe_nested",
        |zip_path| {
            create_zip_archive(zip_path, |writer| {
    ...
    assert_eq!(
        fs::read_to_string(first_file).expect("expected first extracted file"),
        "hello"
    );
    assert_eq!(
        fs::read_to_string(second_file).expect("expected second extracted file"),
        "world"
    );
}
