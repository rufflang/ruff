...
- **Benchmark stage profiling is now opt-in**: `ruff bench-ssg --profile-async` now explicitly enables stage metric emission, while default runs disable stage timing collection in the SSG native hot path to reduce measurement overhead during throughput-only runs.
...
- **Latest throughput reused output-path-buffer step**: `ssg_read_render_and_write_pages(...)` Rayon hot path now reuses per-worker mutable output-path buffers with cached output-file suffix metadata instead of prebuilding full per-batch output-path vectors, reducing output-path allocation pressure while preserving checksum/file-count and stage-metric key contracts.
...
* **Async Batch + Shared State/Task-Pool Contract Follow-Through (P1)**
  ...
  - Added release-hardening contract coverage for task-pool sizing APIs: `set_task_pool_size`, `get_task_pool_size`
  - Added argument/error-shape validation plus success-path behavior checks for async batch read/write lifecycle, shared key lifecycle + int mutation, and task-pool sizing behavior
  - See [CHANGELOG.md](CHANGELOG.md) for implementation and test details
...
* **Random and Time API Contract Follow-Through (P1)**
  ...
  - Hardened strict arity contracts for random/time/date APIs above so trailing arguments now return deterministic contract errors
  - See [CHANGELOG.md](CHANGELOG.md) for implementation and test details
...
* **True Async Runtime Integration (Phase 5 - ✅ 100% COMPLETE!) ⚡**
  ...
  - **Native Async SSG Render+Write Fusion**: `ssg_render_and_write_pages(source_pages, output_dir, concurrency_limit?)` for single-pass render/write throughput optimization ✅
  ...
  - **Examples**:
    ...
    rendered := await ssg_render_and_write_pages(markdown_pages, "tmp/site", 128)
