...
| `ruff test` | Discover `.ruff` fixtures under `tests/`, execute each fixture via `ruff run --interpreter`, and compare output against sibling `.out` snapshots; `--update` regenerates expected-output snapshots. This interpreter pin is intentional until the runtime-path migration tracked in `V1U-RUN-003`. |
