...
pub fn handle(_interp: &mut Interpreter, name: &str, arg_values: &[Value]) -> Option<Value> {
    let result = match name {
        "tcp_listen" => match (arg_values.first(), arg_values.get(1)) {
            (Some(Value::Str(host)), Some(Value::Int(port))) => {
                ...
                match std::net::TcpListener::bind(&address) {
                    ...
                    Err(error) => Value::ErrorObject {
                        message: format!("Failed to bind TCP listener on '{}': {}", address, error),
                        stack: Vec::new(),
                        line: None,
                        cause: None,
                    },
        ...
        "tcp_connect" => match (arg_values.first(), arg_values.get(1)) {
            (Some(Value::Str(host)), Some(Value::Int(port))) => {
                let address = format!("{}:{}", host.as_ref(), port);
        ...
        "tcp_send" => match (arg_values.first(), arg_values.get(1)) {
        ...
        "tcp_receive" => match (arg_values.first(), arg_values.get(1)) {
            (Some(Value::TcpStream { stream, .. }), Some(Value::Int(size))) => {
                if *size <= 0 {
                    return Some(Value::Error("tcp_receive size must be positive".to_string()));
                }
        ...
        "tcp_close" => match arg_values.first() {
            Some(Value::TcpStream { .. }) | Some(Value::TcpListener { .. }) => Value::Bool(true),
            _ => Value::Error("tcp_close requires a TcpStream or TcpListener argument".to_string()),
        },

        "tcp_set_nonblocking" => match (arg_values.first(), arg_values.get(1)) {
            (Some(Value::TcpStream { stream, .. }), Some(Value::Bool(nonblocking))) => {
                ...
                match stream_guard.set_nonblocking(*nonblocking) {
                    ...
                    Err(error) => Value::ErrorObject {
                        message: format!("Failed to set TCP stream non-blocking mode: {}", error),
                        stack: Vec::new(),
                        line: None,
                        cause: None,
                    },
            ...
            (Some(Value::TcpListener { listener, .. }), Some(Value::Bool(nonblocking))) => {
                ...
                match listener_guard.set_nonblocking(*nonblocking) {
                    Ok(_) => Value::Bool(true),
                    Err(error) => Value::ErrorObject {
                        message: format!("Failed to set TCP listener non-blocking mode: {}", error),
                        stack: Vec::new(),
                        line: None,
                        cause: None,
                    },
            ...
            _ => Value::Error(
                "tcp_set_nonblocking requires (TcpStream/TcpListener, bool) arguments".to_string(),
            ),
        },

        "udp_bind" => match (arg_values.first(), arg_values.get(1)) {
            (Some(Value::Str(host)), Some(Value::Int(port))) => {
                let address = format!("{}:{}", host.as_ref(), port);
                match std::net::UdpSocket::bind(&address) {
                    Ok(socket) => {
                        Value::UdpSocket { socket: Arc::new(Mutex::new(socket)), addr: address }
                    }
                    Err(error) => Value::ErrorObject {
                        message: format!("Failed to bind UDP socket on '{}': {}", address, error),
                        stack: Vec::new(),
                        line: None,
                        cause: None,
                    },
        ...
        "udp_receive_from" => match (arg_values.first(), arg_values.get(1)) {
            (Some(Value::UdpSocket { socket, .. }), Some(Value::Int(size))) => {
                ...
                match socket_guard.recv_from(&mut buffer) {
                    Ok((read_size, source_address)) => {
                        ...
                        result.insert(Arc::<str>::from("size"), Value::Int(read_size as i64));
