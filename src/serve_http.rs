...
pub fn run_static_server(dir: PathBuf, host: String, port: u16, options: ServeServerOptions) -> Result<(), String> {
	...
	if tls_enabled && (options.tls_cert.is_none() || options.tls_key.is_none()) {
		return Err("HTTPS requires both --tls-cert and --tls-key to be provided together".to_string());
	}
...
fn planned_to_http_response(plan: PlannedResponse) -> Response<std::io::Cursor<Vec<u8>>> {
	let mut response = Response::from_data(plan.body).with_status_code(StatusCode(plan.status_code));
	if let Some(content_length) = plan.content_length {
		if let Ok(content_length_header) = Header::from_bytes(
...
fn build_response(root_dir: &Path, options: &ServeServerOptions, request: &Request) -> PlannedResponse {
	let method = request.method();
	let is_head = method == &Method::Head;
	if method != &Method::Get && !is_head {
		return headify(text_response(405, "Method Not Allowed", options, request.secure()), is_head);
	}
	...
	if let Some(range_header) = header_value(request.headers(), "Range") {
		match parse_single_range(&range_header, body.len()) {
			...
			Err(_) => {
				...
				add_header(&mut response.headers, "Content-Range", &format!("bytes */{}", file_len));
				add_header(
					&mut response.headers,
					"Content-Type",
...
fn add_default_security_headers(headers: &mut Vec<(String, String)>, options: &ServeServerOptions, secure: bool) {
...
fn status_mapped_error(error: &std::io::Error, options: &ServeServerOptions, secure: bool) -> PlannedResponse {
...
fn text_response(status_code: u16, body: &str, options: &ServeServerOptions, secure: bool) -> PlannedResponse {
